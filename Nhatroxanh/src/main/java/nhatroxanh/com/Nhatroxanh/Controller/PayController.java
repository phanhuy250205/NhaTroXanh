    ...
    public String viewPaymentPage(
        ...
        try {
            ...
            Payments payment = paymentsRepository.findById(paymentIdInt)
                    .orElseThrow(() -> {
                        log.info("Payment not found with id: " + invoiceId + " at " + LocalDateTime.now());
            ...
            if (roomId != null && hostelId != null) {
                room = roomsRepository.findById(roomId)
                        ...
                        .orElseThrow(() -> {
                            log.info("Room not found or hostel mismatch for room_id: " + roomId + " at "
                                    + LocalDateTime.now());
                ...
                Contracts contract = contractsRepository.findById(payment.getContract().getContractId())
                        .orElseThrow(() -> {
                            log.info(
                                    "Contract not found for payment ID: " + invoiceId + " at " + LocalDateTime.now());
                ...
                room = roomsRepository.findById(contract.getRoom().getRoomId())
                        .orElseThrow(() -> {
                            log.info("Room not found for contract ID: " + contract.getContractId() + " at "
                                    + LocalDateTime.now());
                            return new IllegalArgumentException(
            ...
            model.addAttribute("roomPrice", Optional.ofNullable(room.getPrice())
                    ...
                    .orElse("0 VNĐ"));
            model.addAttribute("roomName", Optional.ofNullable(room.getNamerooms()).orElse("Không xác định"));
            ...
            details.forEach(detail -> {
                try {
                    ...
                    log.info("Error processing detail payment item: " + detail.getItemName() + " at "
                            + LocalDateTime.now());
                }
            });
            ...
            log.info("Invalid invoiceId format: " + invoiceId + " at " + LocalDateTime.now());
            model.addAttribute("error", "Mã hóa đơn không hợp lệ: " + invoiceId);
            return "guest/thanh-toan";
        } catch (IllegalArgumentException e) {
            log.info("Entity not found for invoiceId: " + invoiceId + " at " + LocalDateTime.now());
            model.addAttribute("error", e.getMessage());
            return "guest/thanh-toan";
        } catch (Exception e) {
            log.info("Unexpected error loading payment page at " + LocalDateTime.now() + ": " + e.getMessage());
    ...
    public String viewPaymentSuccessPage(
        ...
        try {
            ...
            if (roomId != null && hostelId != null) {
                ...
                Contracts contract = contractsRepository.findById(payment.getContract().getContractId())
                        .orElseThrow(() -> {
                            log.info("Contract not found for payment ID: " + invoiceId + " at " + LocalDateTime.now());
                            return new IllegalArgumentException("Contract not found for payment ID: " + invoiceId);
                        });
                room = roomsRepository.findById(contract.getRoom().getRoomId())
                        .orElseThrow(() -> {
            ...
            model.addAttribute("paymentTime", Optional.ofNullable(payment.getPaymentDate())
                    .map(date -> "10:30 AM") // Replace with actual time if available
                    .orElse("N/A"));
            ...
            log.info("Unexpected error loading payment success page at " + LocalDateTime.now() + ": " + e.getMessage());
            model.addAttribute("error", "Lỗi hệ thống: Vui lòng thử lại sau.");
            return "guest/success-thanhtoan";
        }
    }
