...
public interface PaymentsRepository extends JpaRepository<Payments, Integer> {
       ...
       @Query("SELECT p FROM Payments p WHERE p.contract.room.hostel.owner.userId = :ownerId AND p.paymentStatus = :status ORDER BY p.dueDate DESC")
       Page<Payments> findByOwnerIdAndStatusWithPagination(@Param("ownerId") Integer ownerId,
       ...
       @Query("SELECT DISTINCT p FROM Payments p " +
                     ...
                     ") " +
                     "ORDER BY p.dueDate DESC")
       Page<Payments> searchPaymentsByKeywordWithPagination(@Param("ownerId") Integer ownerId,
       ...
       Optional<Payments> findByContractIdAndMonth(@Param("contractId") Integer contractId, @Param("month") int month,
                     @Param("year") int year);
       ...
       @Query("SELECT p FROM Payments p WHERE (:year IS NULL OR YEAR(p.dueDate) = :year) " +
                     "AND (:status IS NULL OR p.paymentStatus = :status) " +
                     "AND (:method IS NULL OR p.paymentMethod = :method)")
       Page<Payments> findPaymentsByFilters(@Param("year") Integer year,
